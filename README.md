...
The build includes a Cloudflare Pages `_redirects` file:
...
All CSS, JavaScript, image, and video URLs are root-relative, so both `/portfolio`
and `/portfolio/` load the same assets correctly when opened directly or refreshed.
